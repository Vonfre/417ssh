...
- 终端工作区可以点击 `文件` 展开 SFTP 侧栏；侧栏支持将 SFTP 路径复制到终端、同步到终端当前文件夹、自动跟随终端文件夹；目录跳转会先即时切换路径并在后台刷新列表，文件列表支持点击表头排序。
...
- 切换工作区会保留已连接终端和 SFTP 文件浏览状态，终端 SFTP 侧栏的路径、列表、筛选和缓存会按连接分别保存。
