...
- 终端工作区可以点击 `文件` 展开 SFTP 侧栏；侧栏支持将 `cd <SFTP 路径>` 放到终端、主动读取终端 `$PWD` 后同步到当前文件夹、自动跟随终端 `cd` 切换；目录跳转会先即时切换路径并在后台刷新列表，文件列表支持点击表头排序。
- 终端目录同步会保留点号、下划线、空格、括号和中文等路径字符，避免 `/data/project/0_data` 或 `/data/project/2.docker` 这类目录被截断；路径同步按钮使用和 macOS 对应的图标按钮。
- 配置窗口和 SFTP 工作区继续向 macOS 版对齐：完成按钮使用明确主色，端口字段不再显示无用上下箭头，SFTP 多窗口新增按钮更醒目。
- 启动链路更轻：Qt 版不会加载旧 Tk 界面和 Pillow；Qt WebEngine 改为按需加载，只有打开内置网页或真实终端时才初始化。当前 Python + PySide6 + QtWebEngine 方案会天然带来较大的 portable 包，进一步瘦身建议后续改走原生 .NET/WinUI + 系统 OpenSSH/WinSCP 路线。
...
- 设置里可以检查 GitHub Releases 更新；检测到新版本后会显示下载进度、下载目录和安装日志，下载 portable `.zip`，交给后台 updater 在主程序退出后自解压、替换当前 portable 文件夹并重启，不会弹出命令行窗口。
- Windows 自动更新会先备份当前 portable 文件夹，再清空旧目录并复制新版；如果替换失败，会尝试回滚并重启原版本，并在设置页显示上次安装失败原因和日志路径。
