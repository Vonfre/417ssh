...
- 设置里可以检查 GitHub Releases 更新；检测到新版本后会显示下载进度，下载 portable `.zip`，交给后台 updater 在主程序退出后自解压、替换当前 portable 文件夹并重启，不会弹出命令行窗口。
- Windows 自动更新会先备份当前 portable 文件夹，再清空旧目录并复制新版；如果替换失败，会尝试回滚并重启原版本。
