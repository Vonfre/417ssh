...
- 提供内置终端，也支持打开系统原生终端。
- 提供单个纯 SFTP 工作区，工作区内用顶部标签切换多个 SFTP 会话；默认标签连接本地主机 `127.0.0.1`，新标签可从 Hosts 页面选择本地、终端工作区或自定义 SFTP 来源。
- 浏览本地和远程目录，支持本地拖到远程上传、远程拖到本地下载、远程标签间传输、下载、打开文件，以及右键复制、重命名、删除、新建文件夹和修改权限。
- 终端工作区的文件浏览器作为可展开侧栏显示，目录跳转会先即时切换路径并在后台刷新，文件列表支持点击表头排序，也支持拖拽传输。
