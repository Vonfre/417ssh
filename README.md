...
- Windows 终端页支持 Ctrl+C 中断和打开系统原生 `ssh.exe`；如果内置终端遇到兼容性问题，可以直接用原生终端接管当前配置。
...
- 终端工作区的文件浏览器作为可展开侧栏显示，支持将 `cd <SFTP 路径>` 放到终端、主动读取终端 `$PWD` 后同步到当前文件夹、自动跟随终端 `cd` 切换；目录跳转会先即时切换路径并在后台刷新，文件列表支持点击表头排序，也支持拖拽传输。
- 终端当前目录同步会保留点号、下划线、空格、括号和中文等路径字符，避免 `/data/project/0_data` 或 `/data/project/2.docker` 被误识别成 `/data/project/0`、`/data/project/2`。
- Windows 版会自动把端口转发目标里的 `0.0.0.0` / `::` 修正为 `127.0.0.1`；如果 SSH 目标主机误填为这些未指定地址，会给出中文提示而不是底层英文异常。Windows 内置转发遇到 `-g` 时会回退到 `127.0.0.1` 本机监听，避免 SSH.NET 把 `0.0.0.0` 作为 host 后直接连接失败。
...
- Windows 版本以 portable zip 文件夹分发；解压后运行 `417ssh.exe`，后续可在设置里直接更新。更新包会下载到当前 portable 目录下的 `.417ssh-updates`，下载完成后自动退出、解压 zip 里的 `417ssh` 文件夹、替换当前 portable 文件夹内容、重启并清理更新包。
